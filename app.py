...
AI Measure Pro YOLO - Version 5.1
"""
...
class GlassButton(tk.Button):
    def __init__(self, parent, accent=None, **kwargs):
        ...
        super().__init__(
            parent, relief="flat", bd=0, cursor="hand2",
            bg=C["btn"], fg=C["white"], activebackground=C["btn_hover"],
            activeforeground=C["white"], font=("Segoe UI", 10, "bold"),
...
class AIMeasureV5App:
    def __init__(self, root):
        ...
        self.voice = VoiceAnnouncer(enabled=saved_voice_state, rate=150, volume=0.9)
        ...
        self.tracker = ObjectTracker(max_age=30, max_history=20)
    ...
    def build_ui(self):
        ...
        self.overlay_label = tk.Label(left, text=" Camera OFF - Click Start Camera",
    ...
    def _draw_accuracy_bar(self, pct, label="N/A"):
    ...
    def _set_detection_text(self, content):
    ...
    def start_camera(self):
        ...
        if self.pixels_per_cm:
            ...
            self.overlay_label.config(text=" Calibration restored", fg=C["green"])
        else:
            self._update_stat("stat_mode", "EST")
            self.overlay_label.config(text=" Estimated mode - Show bank card", fg=C["yellow"])
        self.update_frame()
    ...
    def recalibrate(self):
        ...
        self._update_stat("stat_mode", "EST")
        self.overlay_label.config(text=" Recalibrating... Show bank card", fg=C["yellow"])
        self.soc_logger.log_calibration("RESET")
        if self.voice.enabled:
            self.voice.announce_calibration("reset")
        messagebox.showinfo("Recalibrate", "Calibration reset.\nShow your bank card clearly in the frame.")
    ...
    def save_measurement(self):
        ...
        self.load_history_to_table()
        messagebox.showinfo("Saved", f"Saved {len(self.last_measurements)} measurement(s).")
